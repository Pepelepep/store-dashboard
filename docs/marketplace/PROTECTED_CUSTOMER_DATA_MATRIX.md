...
| App permissions | App-created data and current Shopify session identity | User email, Shopify user ID from current session where available, role, location access, can view/manage | App access control | No customer data; staff personal data may be stored | Active install; proposed 30 days after uninstall unless deletion request | Deleted by `shop/redact`; open decision whether uninstall should reset | Medium | `user_email` is a staff/app permission identity, not customer email. Public app permissions use Shopify session identity plus ShopOps Studio DB access rules |
