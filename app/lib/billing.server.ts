...
export type CapacityResource = "active_locations" | "dashboard_users";
...
export function getDashboardUserIdentityKey(
  row: Omit<DashboardUserRow, "id"> & { id?: string },
) {
  const email = row.user_email?.trim().toLowerCase();
  if (email) return `email:${email}`;
  const accessLabel = row.access_label?.trim().toLowerCase();
  if (accessLabel) return `label:${accessLabel}`;
  const shopifyUserId = row.shopify_user_id?.trim();
  if (shopifyUserId) return `shopify:${shopifyUserId}`;
  return row.id ? `missing:${row.id}` : null;
}

export function countActiveDashboardUsers(
  rows: Array<Omit<DashboardUserRow, "id"> & { id?: string }>,
) {
  const identities = new Set<string>();
  for (const row of rows) {
    const key = getDashboardUserIdentityKey(row);
...
export class PlanCapacityError extends Error {
  readonly resource: CapacityResource;
  ...
  constructor({
    ...
    const resourceLabel =
      resource === "active_locations" ? "active location" : "dashboard user";
    const plan = PLAN_DEFINITIONS[planHandle];
    super(
      `${plan.displayName} allows ${limit} ${resourceLabel}${limit === 1 ? "" : "s"}. ${usage} of ${limit} are currently in use.`,
    );
...
export class BillingTemporarilyUnavailableError extends Error {
  constructor() {
    super("Billing is temporarily unavailable. Please retry in a moment.");
    this.name = "BillingTemporarilyUnavailableError";
  }
}

export class BillingAccessRequiredError extends Error {
  constructor() {
    super("An active ShopOps Studio plan is required.");
    this.name = "BillingAccessRequiredError";
  }
}

export function logPlanLimitRejection({
  shop,
  billingState,
  planHandle,
  resource,
  ...
  shop: string;
  billingState: AccessibleBillingState["state"];
  planHandle: PlanHandle;
  resource: CapacityResource;
  usage: number;
  limit: number;
  ...
  logBilling(
    ...
    resource === "active_locations"
      ? "location-limit rejection"
      : "dashboard-user-limit rejection",
    {
      shop: normalizeShopDomain(shop),
      billingState,
      plan: planHandle,
...
export async function requirePlanCapacity({
  ...
  if (requestedIncrease <= 0) {
    return { enforced: false, limit: null, billing: null } as const;
  }
  ...
  if (billing.state === "disabled") {
    return { enforced: false, limit: null, billing } as const;
  }
  if (billing.state === "billing_unavailable") {
  ...
  const limit =
    resource === "active_locations"
      ? limits.activeLocations
      : limits.dashboardUsers;
  if (limit !== null && currentUsage + requestedIncrease > limit) {
  ...
  return { enforced: true, limit, billing } as const;
}

export function applyActiveLocationLimit({
  incoming,
  existingActiveIds,
  limit,
}: {
  incoming: Array<{ id: string; isActive: boolean }>;
  existingActiveIds: Iterable<string>;
  limit: number | null;
}) {
  const incomingById = new Map(
    incoming.map((location) => [location.id, location]),
  );
  const preservedActiveIds = new Set(
    Array.from(existingActiveIds).filter(
      (id) => incomingById.get(id)?.isActive === true,
    ),
  );
  const activeIds = new Set(preservedActiveIds);
  const candidates = incoming
    .filter(
      (location) => location.isActive && !preservedActiveIds.has(location.id),
    )
    .sort((a, b) => a.id.localeCompare(b.id));
  const blockedIds: string[] = [];

  for (const location of candidates) {
    if (limit === null || activeIds.size < limit) {
      activeIds.add(location.id);
    } else {
      blockedIds.push(location.id);
    }
  }
